...
export function ExplorerTopNav() {
  ...
  const fallbackHref = from === "library" ? "/library" : "/";
  const backLabel = from === "library" ? "חזרה" : "חזרה לתפריט";
  return (
    <nav
      className="mb-6 flex w-full gap-3"
      aria-label="ניווט מסך מגלה המזונות"
    >
      <BackSmartButton className={btnClass} fallbackHref={fallbackHref}>
        <span className="shrink-0 text-lg leading-none" aria-hidden>
          🏠
        </span>
        <span className="truncate text-center leading-tight">{backLabel}</span>
      </BackSmartButton>
