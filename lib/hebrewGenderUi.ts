...
export function dailyCalorieMotivationLine(
  ...
  const femaleBoosters = [
    "התמדה כזאת זה כוח — את אלופה.",
    "תיעוד מדויק = תוצאה מדויקת. את בדרך הנכונה.",
    "ידע זה כוח — וכל רשומה שלך מקרבת אותך לגוף החלומות.",
    "איזו שליטה. המשיכי ככה — זה בדיוק זה.",
    "כל בחירה חכמה מצטברת. את בונה ניצחון.",
    "עם התמדה כזאת, את מתקרבת לגוף החלומות שלך במהירות האור!",
    "תחשבי על תחושת הסיפוק כשתמדדי את הג׳ינס שחיכה בארון.",
    "את ממש נוגעת ביעד. המשיכי כך!",
  ] as const;
  const maleBoosters = [
    "עבודה של מקצוען. אתה בדרך הבטוחה למבנה הגוף שתמיד רצית.",
    "דיוק של מכונה! עוד קצת ואתה כובש את היעד.",
    "התמדה כזאת בונה גוף חזק וביטחון של מנצח.",
    "ידע זה כוח — וכל רשומה שלך שווה תוצאה.",
    "שליטה אמיתית. המשך ככה.",
    "אתה בונה מומנטום. אל תעצור.",
  ] as const;
  ...
  if (total === 0) {
    return gf(
      gender,
      `היום רק התחיל — כל בחירה תקרב אותך ליעד היומי. ${pick(femaleBoosters)}`,
      `היום רק התחיל — כל בחירה תקרב אותך ליעד היומי. ${pick(maleBoosters)}`
    );
  }
  ...
  if (pct > 100) {
    ...
    return gf(
      gender,
      `היום על ${p}% מהיעד — וזה קורה. את עדיין אלופה; מחר חוזרים חזק. ${pick(femaleBoosters)}`,
      `היום על ${p}% מהיעד — וזה קורה. אתה עדיין אלוף; מחר חוזרים חזק. ${pick(maleBoosters)}`
    );
  }
  ...
  if (pct === 100) {
    return gf(
      gender,
      `פגעת בול ביעד היומי — דיוק של אלופה. ${pick(femaleBoosters)}`,
      `פגעת בול ביעד היומי — דיוק של אלוף. ${pick(maleBoosters)}`
    );
  }
  ...
  if (pct >= 75) {
    return gf(
      gender,
      `את ב־${pct}% מהדרך ליעד היומי! התקדמות מעולה. ${pick(femaleBoosters)}`,
      `אתה ב־${pct}% מהדרך ליעד היומי! התקדמות מעולה. ${pick(maleBoosters)}`
    );
  }
  ...
  if (pct >= 50) {
    return gf(
      gender,
      `את כבר מעבר לחצי — ${pct}% מהדרך ליעד. את מנהלת את זה כמו מקצוענית. ${pick(femaleBoosters)}`,
      `אתה כבר מעבר לחצי — ${pct}% מהדרך ליעד. אתה מנהל את זה כמו מקצוען. ${pick(maleBoosters)}`
    );
  }
  ...
  if (pct >= 25) {
    return gf(
      gender,
      `את ב־${pct}% מהדרך ליעד — התחלה חזקה. ${pick(femaleBoosters)}`,
      `אתה ב־${pct}% מהדרך ליעד — התחלה חזקה. ${pick(maleBoosters)}`
    );
  }

  return gf(
    gender,
    `את ב־${pct}% מהדרך ליעד — יצאת לדרך. ${pick(femaleBoosters)}`,
    `אתה ב־${pct}% מהדרך ליעד — יצאת לדרך. ${pick(maleBoosters)}`
  );
}
